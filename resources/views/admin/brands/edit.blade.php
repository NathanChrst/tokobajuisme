<x-admin-layout>
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8 pb-4 border-b border-[#232336] max-w-2xl mx-auto">
        <div>
            <h1 class="font-serif font-bold text-2xl sm:text-3xl text-white tracking-tight">Edit Brand</h1>
            <p class="text-xs sm:text-sm text-slate-400 mt-1">{{ $brand->name }}</p>
        </div>
        ...
        <form action="{{ route('admin.brands.update', $brand) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="space-y-5">
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $brand->name) }}" required class="w-full rounded-xl bg-[#161622] border border-[#232336] text-white placeholder-slate-500 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/25 shadow-sm text-sm">
                ...
                <div>
                    ...
                    @if($brand->logo)
                        <div class="mb-4 p-2 bg-[#161622] border border-[#232336] rounded-xl inline-block">
                            <img src="{{ asset('storage/' . $brand->logo) }}" alt="{{ $brand->name }}" class="h-16 object-contain">
                        </div>
                    @endif
                    ...
                    <p class="text-xs text-slate-500 mt-2">Upload gambar baru jika ingin mengganti logo. Maksimal 2MB (JPG, PNG)</p>
